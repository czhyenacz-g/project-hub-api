 ...
 * Only the five named `hardcore*` keys are ever read out of `raw` — every
 * other key (unknown or Normal-like, e.g. `totalDeaths`) is never even
 * looked at, let alone stored. `hardcoreDeathsByNight` is the ONLY
 * permitted death histogram — a Normal death histogram has no field name
 * that would ever be read here.
 */
